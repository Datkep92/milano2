...
function loadTodayData() {
  ...
  // Lấy element mới nhất từ DOM
  const bankInputEl = document.getElementById("bankInput");
  const cashInputEl = document.getElementById("cashInput");
  const reserveInputEl = document.getElementById("reserveInput");
  const revenueInputEl = document.getElementById("revenueInput");
  const grabInputEl = document.getElementById("grabInput");
  
  if (bankInputEl) bankInputEl.value = formatNumberForInput(report.bank || 0);
  if (cashInputEl) cashInputEl.value = formatNumberForInput(report.cash || 0);
  if (reserveInputEl) reserveInputEl.value = formatNumberForInput(report.reserve || 0);
  if (revenueInputEl) revenueInputEl.value = formatNumberForInput(report.revenue || 0);
  if (grabInputEl) grabInputEl.value = formatNumberForInput(report.grab || 0);
  
  // Các ô readonly (không bị clone)
